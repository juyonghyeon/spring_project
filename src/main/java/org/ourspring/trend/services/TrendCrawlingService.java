...
import org.ourspring.trend.entities.TrendCrawling;
import org.ourspring.trend.repositories.TrendRepository;
import org.ourspring.trend.repositories.TrendUrlRepository;
...
public class TrendCrawlingService {
    ...
    public TrendCrawling process(String search){
        ...
        try {
            ...
            if (process.waitFor() == 0) { // 정상 수행된 경우
                builder = new ProcessBuilder(pythonPath, properties.getTrend() + "/trend.py", fileProperties.getPath() + "/trend", search);
                ...
                if (statusCode == 0) {
                    ...
                    try {
                        ...
                        data.setCategory(search.contains("news.naver.com") ? "NEWS" : "ETC_" + Objects.hash(search));
